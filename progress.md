...
### RISC-V RVV Backend ✅ (commits 4266359, 3940026, 6bb641e, 70a6662, 1dae1f2, 2179c4a)

**Full RISC-V backend**: IR → C + RVV vector intrinsics codegen, execution via QEMU CI.

**Architecture**: RISC-V CPU hosting NVIDIA GPU over PCIe (CUDA backend use case) + RVV CPU vector units (this backend).

**Files added/modified**:
- `locomp/backends/riscv_codegen.py` — IR → C + RVV intrinsics (`__riscv_vle32`, `__riscv_vse32`, `__riscv_vfadd`, `__riscv_vfmul`, etc.)
- `locomp/api.py` — `backend="riscv"`, `_launch_riscv()`, `auto` → `riscv` on non-macOS
- `tests/test_riscv_codegen.py` — 19 codegen string-check tests
- `tests/test_riscv_execution.py` — 9 QEMU execution tests (cross-compile → qemu-riscv64-static → compare to NumPy)
- `tests/conftest.py` — `macos_only` pytest mark
- `.github/workflows/riscv.yml` — 2-job CI: unit tests + QEMU execution (ubuntu + `gcc-riscv64-linux-gnu` + `qemu-user-static`)

**Bugs fixed**:
1. **Scalar pointer ADD**: IR uses `ADD` opcode (not `PTR_ADD`) for pointer arithmetic → was emitting `float tmp = A + i` instead of `float* tmp = A + i`. Fixed in `_gen_arith_binop`.
2. **Tiled pointer ADD**: `A + i*64 + arange(0,64)` → `_ptr_exprs` now stores 3-tuple `(base, idx, is_tiled)`. When `is_tiled=True`, `_gen_load`/`_gen_store` emit contiguous RVV `vle`/`vse` from `base` (arange offsets are always `[0..N-1]`).
3. **Missing `sym` variable**: `_gen_arith_binop` fell through to scalar `f"{a} {sym} {b}"` before `sym` was defined. Fixed by adding opcode→symbol dict before tiled/scalar paths.
4. **`compile_to_riscv` NameError on Linux**: was only locally imported inside `_launch_riscv()`, but `_compile()` used it at module scope. Fixed by top-level import in `api.py`.
5. **Metal tests on Linux**: 27 tests calling `.msl` or launching Metal GPU skipped with `@macos_only` mark on Linux CI.

**CI status** (commit `2179c4a`):
- `RISC-V RVV Tests` → `RISC-V RVV Execution (QEMU)`: ✅ success, `Unit Tests (codegen + IR)`: ✅ success
- `Test locomp` (macOS Metal): ✅ success
- 95 tests pass locally on macOS, 9 QEMU execution tests pass on Linux CI

**Current version**: v0.6.0 on PyPI. v0.7.0 ready to publish (RISC-V backend). PyPI token in `.env` (gitignored).

### CUDA wmma Tensor Core Codegen ✅ (commits b6cb222, 34d496e)

**Real `nvcuda::wmma` codegen** — replaced the stub comment with full implementation.

**Files modified**:
- `locomp/backends/cuda_codegen.py` — `_uses_wmma()`, `_wmma_fragment_type()`, real `_gen_op()` wmma branch
- `tests/test_cuda_codegen.py` — 4 new wmma tests (all passing)
- `tests/modal_cuda_runner.py` — pinned to commit `34d496e`

**Codegen mapping**:
| IR OpCode | CUDA output |
|-----------|-------------|
| `SIMDGROUP_MATRIX_FILL` | `wmma::fill_fragment(rv, val)` |
| `SIMDGROUP_MATRIX_LOAD` | `wmma::load_matrix_sync(rv, ptr+offset, stride)` |
| `SIMDGROUP_MATRIX_STORE` | `wmma::store_matrix_sync(ptr+offset, mat, stride, layout)` |
| `SIMDGROUP_MATRIX_MAC` | `wmma::mma_sync(rv, a, b, acc)` |

**Fragment types**: 16×16×16 fp16 tiles. `role="acc"` → `wmma::accumulator, float`. `role="a"/"b"` → `matrix_a/matrix_b` with row/col major layout.

**Auto-include**: `#include <mma.h>` + `using namespace nvcuda;` conditionally added only when wmma ops are present (`_uses_wmma()` checks IR opcodes).

**Test results**: 17/17 CUDA codegen tests pass, 112/112 total tests pass (9 skipped).

**Bug fixed**: Test was calling `locomp.simdgroup_multiply_accumulate()` — Metal API name. CUDA/common API name is `locomp.simdgroup_mac()`. One-line fix in the test.

### v0.7.0 Release ✅ (5 April 2026)

**Published to PyPI**: https://pypi.org/project/locomp/0.7.0/

**Changes since v0.6.0**:
- RISC-V RVV backend (6 commits)
- CUDA backend: wmma tensor core codegen (`mma_sync`, `load/store_matrix_sync`, `fill_fragment`)
- CUDA wmma `role` kwarg on `simdgroup_matrix_load_device` — correctly types `matrix_a`/`matrix_b` fragments for `nvcc` type checking
- **Autotune backend-agnostic**: `_get_gpu_name()` now tries Metal first, then `nvidia-smi`, then falls back. Float constexpr values preserved (no more `int(arg)` truncation). Tensor detection no longer requires `_metal_buffer` — works for NumPy arrays and CUDA tensors.
- **CUDA wmma A10G benchmark**: 1024³ fp16 matmul via `wmma::mma_sync` in modal runner, reports TFLOPS
- **GitHub Actions CI**: `MODAL_TOKEN_ID` + `MODAL_TOKEN_SECRET` set as repo secrets — `cuda-execution` job now runnable via `workflow_dispatch`

**Commits**:
- `b6cb222` — real CUDA wmma codegen
- `78f9208` — v0.7.0 (role kwarg, autotune, wmma benchmark, version bump)
- `4557fb2` — modal runner pinned to 78f9208
- `5da4129` — fix: wmma fragment pre-declaration + Float16 annotation
- `9c7fe48` — modal runner pinned to 5da4129
- `3bce48d` — docs: CUDA A10G benchmark results in README
- `39a4dd1` — feat: autograd (locomp.ag) + autotune unit tests

### Autotune Unit Tests ✅ (commit 39a4dd1)

13 tests covering: `Config` static/derived constexprs, grid resolution, `AutotunedKernel` config selection, in-memory cache, different keys benchmarked separately, disk cache round-trip (monkeypatched), `clear_cache()`, float constexpr preserved (not truncated to int), Metal GELU integration test on M1.

### Autograd ✅ (commit 39a4dd1)

**`locomp/autograd.py`** — tape-based reverse-mode autodiff. Exposed as `locomp.ag`.

**Supported ops**: `add`, `sub`, `mul`, `div`, `matmul`, `exp`, `log`, `relu`, `pow`, `sum`, `mean`, `sigmoid`, `tanh`, `softmax`, `cross_entropy`
...
c = locomp.ag.add(a, b)
loss = locomp.ag.sum(c)
locomp.ag.backward(loss)
print(a.grad)  # dL/da
```

**`tests/test_autograd.py`** — 34 tests:
- `AGTensor` creation, numpy, item, requires_grad
- `no_grad` context manager disables/restores tape  
- Forward correctness: add, mul, matmul, relu, softmax
- Backward correctness (vs finite differences, atol=1e-2): add, sub, mul, div, exp, log, relu, pow, sigmoid, tanh
- Exact gradients: matmul dA/dB, sum, mean, chain rule (a+b)*c
- MLP forward+backward: 2-layer relu network, gradients finite + correct shape
- Cross-entropy forward/backward
- Tape cleared after `backward()`
- `zero_grad()` resets `.grad` to None

**Current test count**: 159 passed, 9 skipped (was 112 before this session)

### GPU Autograd ✅ (commit d3738da)

**`locomp/gpu_autograd.py`** — GPU-backed tape-based reverse-mode autodiff. Exposed as `locomp.gpu_ag`.

Forward and backward passes execute as real locomp kernels on Metal/CUDA. Gradients stay on-device until `.numpy()` is called.

**Supported ops**: `add`, `sub`, `mul`, `div`, `exp`, `log`, `relu`, `sum`, `mean`, `matvec`

**Architecture**:
- `GPUTensor` wraps `LocompTensor` — grad is also a `GPUTensor` (on-device)
- Kernel registry `_kc` — lazily compiled per `(name, backend)` pair
- Tape records `(output, backward_fn, inputs)` — reversed on `backward()`
- `_bwd_broadcast` kernel — scalar gradient broadcast using float constexpr (avoids `ptr+0` optimizer bug)
- `no_grad()` context manager, `zero_grad()`, tape cleared after `backward()`
...
**`tests/test_gpu_autograd.py`** — 36 tests:
- `GPUTensor` creation, numpy roundtrip, requires_grad
- Forward correctness on M1: add, sub, mul, div, exp, log, relu, sum, mean, matvec
- Backward correctness on M1: all ops, chain rule (relu(exp(x))), no_grad, tape cleared
- CUDA codegen tests (no GPU): verify backward kernel `.cu` source strings for all backward kernels
- Namespace test: `locomp.gpu_ag` accessible with `.backward`, `.tensor`, `.GPUTensor`

**Bugs fixed during implementation**:
1. `from __future__ import annotations` turned annotations to strings — broke `ann is constexpr` check in frontend
2. Circular import (`locomp.__init__` → `gpu_autograd` → `locomp.Tensor`) — fixed by importing directly from `locomp.frontend`
